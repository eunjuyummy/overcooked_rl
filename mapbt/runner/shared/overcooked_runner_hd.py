...
from mapbt.runner.shared.base_runner import Runner
...
class OvercookedRunner(Runner):
    ...
    def behavior_cloning(self, training_data):
        ...
        policy_model = self.trainer.policy.actor 

        num_epochs = self.all_args.bc_num_epochs
        best_validation_acc = 0.
        best_sparse_r, best_sparse_r_epoch = -1e9, 0
        ...
        for epoch in range(num_epochs):
            ...
            validation_acc = validation_info["validation_accuracy"]    
            ...
            if epoch % self.eval_interval == 0:
                ...
                if sparse_r > best_sparse_r:
                    ...
                    best_sparse_r_epoch = epoch
                    torch.save(policy_model.state_dict(), str(self.save_dir) + "/policy_best_sparse_r.pt")
                if sparse_r_agent0 > best_sparse_r_agent0:
                    best_sparse_r_agent0 = sparse_r_agent0
                    torch.save(policy_model.state_dict(), str(self.save_dir) + "/policy_best_sparse_r_agent0.pt")
                if sparse_r_agent1 > best_sparse_r_agent1:
                    best_sparse_r_agent1 = sparse_r_agent1
                    torch.save(policy_model.state_dict(), str(self.save_dir) + "/policy_best_sparse_r_agent1.pt")

            if (epoch % self.save_interval == 0 or epoch == num_epochs):
                torch.save(policy_model.state_dict(), str(self.save_dir) + f"/policy_epoch{epoch}.pt")
        
        print("best_validation_acc", best_validation_acc)
        print("best_sparse_r", best_sparse_r, "best_sparse_r_epoch", best_sparse_r_epoch)
